...
export default function AdminConsole() {
  ...
  async function saveGalleryVideo() {
    ...
    setGalleryMessage(editingGalleryId ? "修改已保存，前台刷新后即可查看 ✓" : "视频灵感已保存，前台刷新后即可查看 ✓"); setEditingGalleryId(""); setGalleryVideo(undefined); setGalleryPoster(undefined); setGalleryForm({ title:"",category:"响指变装",description:"",prompt:"",sortOrder:galleryVideos.length + 2,enabled:true }); refreshGallery();
  }
  ...
  function editGalleryVideo(video: {id:string;title:string;category:string;description:string;prompt:string;sortOrder:number;enabled:boolean}) {
    setEditingGalleryId(video.id); setGalleryForm({title:video.title,category:video.category,description:video.description,prompt:video.prompt,sortOrder:video.sortOrder,enabled:video.enabled}); setGalleryVideo(undefined); setGalleryPoster(undefined); setGalleryMessage("正在编辑现有视频，可只修改资料，也可以选择新文件进行替换"); window.scrollTo({top:0,behavior:"smooth"});
  }
  function cancelGalleryEdit() { setEditingGalleryId(""); setGalleryVideo(undefined); setGalleryPoster(undefined); setGalleryForm({title:"",category:"响指变装",description:"",prompt:"",sortOrder:galleryVideos.length + 1,enabled:true}); setGalleryMessage(""); }
    ...
    <section className="admin-main"><header className="admin-topbar"><div><span>COMMERCE CONTENT OS</span><h1>{active === "overview" ? "运行概览" : active === "connections" ? "服务接入" : active === "prompts" ? "生成策略" : active === "templates" ? "视频模板" : active === "gallery" ? "视频灵感库" : "接口诊断"}</h1></div><div className="admin-user"><span>LY</span><div><b>内容管理员</b><small>Administrator</small></div></div></header>
      ...
      {active === "gallery" && <div className="admin-content gallery-manager">
        ...
        <div className={`template-admin gallery-admin ${editingGalleryId ? "is-editing" : ""}`}>
          ...
          <section><header className="gallery-editing-header"><b>{editingGalleryId ? "正在修改已有视频" : "新增视频灵感"}</b>{editingGalleryId && <button onClick={cancelGalleryEdit}>取消修改</button>}</header><div className="template-form-row"><label><span>视频名称</span><input value={galleryForm.title} onChange={e=>setGalleryForm({...galleryForm,title:e.target.value})}/></label><label><span>分类标签</span><input value={galleryForm.category} onChange={e=>setGalleryForm({...galleryForm,category:e.target.value})}/></label></div><div className="template-form-row"><label><span>简短说明</span><input value={galleryForm.description} onChange={e=>setGalleryForm({...galleryForm,description:e.target.value})}/></label><label><span>展示顺序</span><input type="number" min="1" max="6" value={galleryForm.sortOrder} onChange={e=>setGalleryForm({...galleryForm,sortOrder:Number(e.target.value)})}/></label></div><label><span>用户可复制的完整提示词</span><textarea value={galleryForm.prompt} onChange={e=>setGalleryForm({...galleryForm,prompt:e.target.value})}/></label><div className="template-form-actions"><label><input type="checkbox" checked={galleryForm.enabled} onChange={e=>setGalleryForm({...galleryForm,enabled:e.target.checked})}/> 在前台启用</label><button onClick={saveGalleryVideo}>{editingGalleryId ? "保存全部修改" : "上传并发布"}</button></div>{galleryMessage && <p className="template-message">{galleryMessage}</p>}</section>
        </div>
        <div className="gallery-management-grid">{Array.from({length:6},(_,index)=>galleryVideos[index] || null).map((video,index)=>video ? <article key={video.id} className={video.enabled ? "active" : "inactive"}><div className="gallery-admin-media">{video.videoUrl ? <video src={video.videoUrl} poster={video.posterUrl} muted playsInline controls preload="metadata"/> : <div className="template-empty">VIDEO</div>}<span>位置 0{index+1}</span></div><div className="gallery-admin-info"><small>{video.category} · 排序 {video.sortOrder}</small><h3>{video.title}</h3><p>{video.description}</p></div><div className="gallery-admin-actions"><button onClick={()=>editGalleryVideo(video)}>编辑资料</button><label><input type="file" accept="video/mp4,video/webm,video/quicktime" onChange={event=>{const file=event.target.files?.[0];if(file)replaceGalleryAsset(video,file,"video");event.currentTarget.value=""}}/>替换视频</label><label><input type="file" accept="image/jpeg,image/png,image/webp" onChange={event=>{const file=event.target.files?.[0];if(file)replaceGalleryAsset(video,file,"poster");event.currentTarget.value=""}}/>替换封面</label><button className={video.enabled ? "enabled" : "disabled"} onClick={()=>toggleGalleryVideo(video)}>{video.enabled ? "已启用" : "已停用"}</button><button className="gallery-delete" onClick={()=>deleteGalleryVideo(video.id)}>删除</button></div></article> : <article className="empty-slot" key={`slot-${index}`}><div><span>＋</span><b>位置 0{index+1}</b><p>上传一个新视频后将自动进入此位置</p></div></article>)}</div>
      </div>}
    </section>
  </main>;
}
