...
export default function AdminConsole() {
  ...
  const [galleryForm, setGalleryForm] = useState({ title: "", code: "", category: "响指变装", description: "", prompt: "", lookCount: 5, sortOrder: 1, enabled: true });
  const [galleryMessage, setGalleryMessage] = useState("");
  const [editingGalleryId, setEditingGalleryId] = useState("");
  const [galleryVideos, setGalleryVideos] = useState<Array<{id:string;title:string;code?:string;category:string;description:string;prompt:string;lookCount?:number;sortOrder:number;enabled:boolean;videoUrl?:string;posterUrl?:string}>>([]);
  ...
  async function saveGalleryVideo() {
    ...
    setGalleryMessage(editingGalleryId ? "修改已保存，前台刷新后即可查看 ✓" : "视频灵感已保存，前台刷新后即可查看 ✓"); setEditingGalleryId(""); setGalleryVideo(undefined); setGalleryPoster(undefined); setGalleryForm({ title:"",code:"",category:"响指变装",description:"",prompt:"",lookCount:5,sortOrder:galleryVideos.length + 2,enabled:true }); refreshGallery();
  }
  ...
  function editGalleryVideo(video: {id:string;title:string;code?:string;category:string;description:string;prompt:string;lookCount?:number;sortOrder:number;enabled:boolean}) {
    setEditingGalleryId(video.id); setGalleryForm({title:video.title,code:video.code || "",category:video.category,description:video.description,prompt:video.prompt,lookCount:video.lookCount || 5,sortOrder:video.sortOrder,enabled:video.enabled}); setGalleryVideo(undefined); setGalleryPoster(undefined); setGalleryMessage("正在编辑现有视频，可只修改资料，也可以选择新文件进行替换"); window.scrollTo({top:0,behavior:"smooth"});
  }
  function cancelGalleryEdit() { setEditingGalleryId(""); setGalleryVideo(undefined); setGalleryPoster(undefined); setGalleryForm({title:"",code:"",category:"响指变装",description:"",prompt:"",lookCount:5,sortOrder:galleryVideos.length + 1,enabled:true}); setGalleryMessage(""); }
    ...
    <section className="admin-main"><header className="admin-topbar"><div><span>COMMERCE CONTENT OS</span><h1>{active === "overview" ? "运行概览" : active === "connections" ? "服务接入" : active === "prompts" ? "生成策略" : active === "gallery" ? "视频灵感库" : active === "pricing" ? "订阅价格" : "接口诊断"}</h1></div><div className="admin-user"><span>LY</span><div><b>内容管理员</b><small>Administrator</small></div></div></header>
      ...
      {active === "gallery" && <div className="admin-content gallery-manager">
        ...
        <div className={`template-admin gallery-admin ${editingGalleryId ? "is-editing" : ""}`}>
          ...
          <section><header className="gallery-editing-header"><b>{editingGalleryId ? "正在修改已有视频" : "新增视频灵感"}</b>{editingGalleryId && <button onClick={cancelGalleryEdit}>取消修改</button>}</header><div className="template-form-row"><label><span>视频名称</span><input value={galleryForm.title} onChange={e=>setGalleryForm({...galleryForm,title:e.target.value})}/></label><label><span>模板编号</span><input placeholder="A/B/C" value={galleryForm.code} onChange={e=>setGalleryForm({...galleryForm,code:e.target.value})}/></label><label><span>分类标签</span><input value={galleryForm.category} onChange={e=>setGalleryForm({...galleryForm,category:e.target.value})}/></label></div><div className="template-form-row"><label><span>简短说明</span><input value={galleryForm.description} onChange={e=>setGalleryForm({...galleryForm,description:e.target.value})}/></label><label><span>展示顺序</span><input type="number" min="1" max="6" value={galleryForm.sortOrder} onChange={e=>setGalleryForm({...galleryForm,sortOrder:Number(e.target.value)})}/></label><label><span>参考图数量</span><input type="number" min="1" max="10" value={galleryForm.lookCount} onChange={e=>setGalleryForm({...galleryForm,lookCount:Number(e.target.value)})}/></label></div><label><span>用户可复制的完整提示词</span><textarea value={galleryForm.prompt} onChange={e=>setGalleryForm({...galleryForm,prompt:e.target.value})}/></label><div className="template-form-actions"><label><input type="checkbox" checked={galleryForm.enabled} onChange={e=>setGalleryForm({...galleryForm,enabled:e.target.checked})}/> 在前台启用</label><button onClick={saveGalleryVideo}>{editingGalleryId ? "保存全部修改" : "上传并发布"}</button></div>{galleryMessage && <p className="template-message">{galleryMessage}</p>}</section>
        </div>
      ...
      {active === "pricing" && <div className="admin-content"><div className="content-heading"><div><span>SUBSCRIPTION PLANS</span><h2>订阅价格</h2><p>管理用户生成视频时的订阅方案和额度，保存后前台订阅弹窗立即生效。</p></div><button onClick={savePricing}>{plansSaved ? "已保存 ✓" : "保存价格"}</button></div><div className="pricing-admin-grid">{plans.map((plan, index) => <article key={plan.id} className={plan.popular ? "popular" : ""}><header><input value={plan.name} onChange={e=>updatePlan(index,"name",e.target.value)}/><label className="popular-toggle"><input type="checkbox" checked={!!plan.popular} onChange={e=>updatePlan(index,"popular",e.target.checked)}/> 最受欢迎</label></header><div className="pricing-admin-row"><label><span>价格</span><input value={plan.price} onChange={e=>updatePlan(index,"price",e.target.value)}/></label><label><span>周期</span><input value={plan.period} onChange={e=>updatePlan(index,"period",e.target.value)}/></label></div><div className="pricing-admin-row"><label><span>白底图额度（-1 不限）</span><input type="number" min="-1" value={plan.whiteQuota} onChange={e=>updatePlan(index,"whiteQuota",Number(e.target.value))}/></label><label><span>真人穿搭额度（-1 不限）</span><input type="number" min="-1" value={plan.hollowQuota} onChange={e=>updatePlan(index,"hollowQuota",Number(e.target.value))}/></label></div><label><span>视频额度（-1 不限）</span><input type="number" min="-1" value={plan.videoQuota} onChange={e=>updatePlan(index,"videoQuota",Number(e.target.value))}/></label><label><span>一句话描述</span><input value={plan.desc} onChange={e=>updatePlan(index,"desc",e.target.value)}/></label><label><span>功能特性（用逗号分隔）</span><textarea value={plan.features.join("，")} onChange={e=>updatePlan(index,"features",e.target.value.split(/[，,]/).map(s=>s.trim()).filter(Boolean))}/></label></article>)}</div></div>}
    </section>
  </main>;
}
