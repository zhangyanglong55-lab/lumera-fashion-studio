...
const requiredAgents = [
  ...
  ["hollow-look", "真人穿搭"], ["virtual-try-on", "真人试穿"], ["snap-change-video", "动态商拍"],
] as const;
...
export function TestWorkspace({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    fetch("/api/prompt-videos").then(response => response.ok ? response.json() : Promise.reject()).then(data => {
      ...
      if (videos.length) {
        const templates = videos.map((video: { id: string; code?: string; title: string; description: string; prompt: string; videoUrl?: string; posterUrl?: string }) => ({
          id: video.id, code: video.code || "NEW", name: video.title, description: video.description, prompt: video.prompt, previewUrl: video.videoUrl, cover: video.posterUrl,
        }));
